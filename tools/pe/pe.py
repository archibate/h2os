...
class Circle:
    ...
    def get_normal(p, pos):
        disp = pos - p.center
        return disp.norm if disp.lengthSquared <= p.radius * p.radius else None

    def render(p, g):
        pygame.draw.circle(g.screen, LIGHT, p.center.int, int(p.radius), 2)
...
game.world.add_static(Circle(center=(game.hwidth, game.hheight), radius=game.hheight * 0.3))
game.mainloop()
